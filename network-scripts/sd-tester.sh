...
network_script="/home/pirat/sd-pfmnc-tester.py"
...
test_nspawn() {
  ...
  if [[ $TIME_OUT -eq 1 ]]; then 
    echo "[FAILED] test nspawn timeout" 1>&2;
  elif [[ $tmp_val -ne 0 ]]; then
    echo "[FAILED] test nspawn return value: $tmp_val"
  fi
  ...
  #success_test "[FAILED] test nspawn machine did not boot correctly" || return 1
...
new_test() {
  ...
  # fun begin
  sysd_compilation || return 1
  rm -rf $NSPAWN_MACHINE
  cp -r $NSPAWN_MACHINE_BACKUP $NSPAWN_MACHINE
  test_nspawn ||  return 1

  init_pfmnc_test || { load_backup_snap && return 1; }
  return 0
}
...
if [[ -e ${LOG_DIR}/check_pfmnc_test ]]; then
  ...
  $network_script --host $HOST_IP $HOST_PORT --test-result $status SEND_RESULT $LOG_DIR/result.tar.xz >> $log_file 2>> $log_file
  status=$?
...
while [[ 1 ]]; do # while is used on error before reboot
  ...
  $network_script --host $HOST_IP $HOST_PORT --test-result $status  SEND_RESULT $LOG_DIR/result.tar.xz
  if [[ $? -ne 0 ]]; then # last chance, server must listen..
    sleep 10
    $network_script --host $HOST_IP $HOST_PORT --test-result $status  SEND_RESULT $LOG_DIR/result.tar.xz
  fi
